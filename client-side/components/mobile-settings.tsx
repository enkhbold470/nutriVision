...
export function MobileSettingsComponent() {
  const settingsOptions: SettingsOption[] = [
    { icon: <Bell className="h-5 w-5 text-green-500" />, label: 'Notifications', toggle: true },
    { icon: <Moon className="h-5 w-5 text-green-500" />, label: 'Dark Mode', toggle: true },
    { icon: <Lock className="h-5 w-5 text-green-500" />, label: 'Privacy', action: () => console.log('Privacy settings') },
    { icon: <HelpCircle className="h-5 w-5 text-green-500" />, label: 'Help & Support', action: () => console.log('Help & Support') },
    ...
    <div className="bg-green-50 min-h-screen p-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-green-700">Enkhbold Ganbold</h2>
            <p className="text-gray-600">enkhbold470@gmail.com</p>
          </div>
        </div>
        <button className="mt-4 text-green-500 font-medium" onClick={() => console.log('Edit profile')}>
          Edit Profile
        </button>
      </div>
        ...
        {settingsOptions.map((option, index) => (
          ...
          >
            ...
            {option.toggle ? (
              ...
              <button onClick={option.action} className="text-green-500">
                &gt;
              </button>
            )}
          </div>
